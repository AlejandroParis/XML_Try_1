...
bool j1App::Awake()
{
	...
	pugi::xml_parse_result result = conf_document.load_file("../Motor2D/config.xml");
	if (result)
	{
		LOG("XML parsed without errors");
		conf_node = conf_document.child("config");
	}
	...
	while(item != NULL && ret == true)
	{
		...
		conf_node = conf_node.child(item->data->name.GetString());
		ret = item->data->Awake(conf_node);
		item = item->next;
	}
